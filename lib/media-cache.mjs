...
async function ensureDownloaded(videoId, audioOnly, meta, key) {
  if (await isCached(key)) return readMeta(key);
...
export async function serveMedia(videoId, audioOnly, req, res) {
  ...
  try {
    await ensureDownloaded(videoId, audioOnly, meta, key);
  } catch (err) {
  ...
  if (await isCached(key)) {
    const cachedMeta = await readMeta(key);
    serveFile(req, res, cachePaths(key).media, cachedMeta);
