...
export async function forwardMediaRequest(videoId, audioOnly, req, res) {
    ...
    if (!res.headersSent) {
      sendJson(res, 502, {
        error: err.name === 'TimeoutError'
          ? 'Media backend timed out — it may be waking up, try again in 30s'
          : 'Media backend unreachable',
      });
    }
    return false;
  }
  ...
  if (!upstream.ok) {
    let error = `Media backend error (${upstream.status})`;
    try {
    ...
    if (!res.headersSent) sendJson(res, 502, { error });
    return false;
  }
